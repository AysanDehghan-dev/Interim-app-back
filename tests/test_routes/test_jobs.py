...
def test_apply_for_job_unauthorized(client, company_auth_headers, test_job):
    ...
    application_data = {
        'coverLetter': 'This application should not be created'
    }
...
def test_apply_for_job_already_applied(client, auth_headers, test_job, test_application):
...
def test_get_job_applications(client, company_auth_headers, test_job, test_application):
    ...
    response = client.get(
        f'/api/jobs/{test_job["_id"]}/applications',
        headers=company_auth_headers
    )
