...
def test_get_profile_with_company_token(client, company_auth_headers):
    ...
    response = client.get("/api/users/profile", headers=company_auth_headers)
...
def test_add_experience(client, auth_headers):
    ...
    experience = next(
        (exp for exp in data["experience"] if exp["title"] == "Software Developer"),
        None,
    )
...
def test_add_education(client, auth_headers):
    ...
    education = next(
        (
            ...
            if edu["institution"] == "University of Technology"
        ),
        None,
    )
