...
class Application:
    ...
    def create(application_data):
        ...
        if "job_id" in application_data and not isinstance(
            application_data["job_id"], ObjectId
        ):
            application_data["job_id"] = ObjectId(application_data["job_id"])

        if "user_id" in application_data and not isinstance(
            application_data["user_id"], ObjectId
        ):
    ...
    def find_by_user_and_job(user_id, job_id):
        ...
        """
        query = {"user_id": ObjectId(user_id), "job_id": ObjectId(job_id)}
