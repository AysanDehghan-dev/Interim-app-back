...
class Application(BaseModel):
    ...
    def create(cls, application_data):
        ...
        # Validate ObjectIds
        application_data["job_id"] = cls._validate_object_id(
            application_data["job_id"], "job_id"
        )
        application_data["user_id"] = cls._validate_object_id(
            application_data["user_id"], "user_id"
        )

        # Check if user already applied for this job
        existing = cls.find_by_user_and_job(
            application_data["user_id"], application_data["job_id"]
        )
        if existing:
        ...
        if not ApplicationStatus.is_valid(application_data["status"]):
            raise ValidationError(
                f"Invalid status. Must be one of: {', '.join(ApplicationStatus.get_all())}"
            )
