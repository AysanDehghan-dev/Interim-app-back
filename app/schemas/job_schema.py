...
class SalarySchema(BaseSchema):
    ...
    min_salary = fields.Int(
        data_key="min", required=True, validate=validate.Range(min=0)
    )
    max_salary = fields.Int(
        data_key="max", required=True, validate=validate.Range(min=0)
    )
    currency = fields.Str(
        required=True, validate=validate.Length(equal=3)
    )  # USD, EUR, etc.
