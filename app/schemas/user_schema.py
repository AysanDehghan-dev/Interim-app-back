...
from app.schemas.base import BaseSchema, ObjectIdField, PhoneField, TimestampMixin
...
class EducationSchema(BaseSchema):
    ...
    id = fields.Str(dump_only=True)
    school = fields.Str(
        data_key="institution", required=True, validate=validate.Length(min=1, max=100)
    )
    degree = fields.Str(required=True, validate=validate.Length(min=1, max=100))
    field = fields.Str(
        data_key="field_of_study",
        required=True,
...
class UserSchema(BaseSchema, TimestampMixin):
    ...
    first_name = fields.Str(
        data_key="firstName", required=True, validate=validate.Length(min=1, max=50)
    )
    last_name = fields.Str(
        data_key="lastName", required=True, validate=validate.Length(min=1, max=50)
    )
...
class UserUpdateSchema(BaseSchema):
    ...
    first_name = fields.Str(
        data_key="firstName", validate=validate.Length(min=1, max=50)
    )
